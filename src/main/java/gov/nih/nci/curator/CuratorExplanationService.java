...
public class CuratorExplanationService extends ExplanationService {
	...
	public boolean hasExplanation(final OWLAxiom axiom) {
		...
		return axiom.isLogicalAxiom()
		       && manager.getCurrentReasonerFactory() instanceof CuratorReasonerFactory
		       && manager.getCurrentReasonerName().startsWith("NCI Curator")
		       && CuratorReasonerPreferences.getInstance().getExplanationCount() != 0;
	}

	@Override
	public ExplanationResult explain(final OWLAxiom axiom) {
		try {
			int explanationCount =  CuratorReasonerPreferences.getInstance().getExplanationCount();
