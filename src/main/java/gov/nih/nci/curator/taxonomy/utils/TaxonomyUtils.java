...
package gov.nih.nci.curator.taxonomy.utils;
...
import gov.nih.nci.curator.taxonomy.TaxonomyNode;
...
public class TaxonomyUtils {
	...
	public static boolean addSuperExplanation(Taxonomy<OWLClass> t, OWLClass sub, OWLClass sup,
			Set<OWLClass> explanation) {

		Map<OWLClass, Set<Set<OWLClass>>> map = (Map<OWLClass, Set<Set<OWLClass>>>) t.getDatum(
				sub, SUPER_EXPLANATION_KEY );
		Set<Set<OWLClass>> explanations;
		if( map == null ) {
			if( t.contains( sub ) ) {
				map = new HashMap<OWLClass, Set<Set<OWLClass>>>();
		...
		if( explanations == null ) {
			explanations = new HashSet<Set<OWLClass>>();
	...
	public static void clearSuperExplanation(Taxonomy<OWLClass> t, OWLClass c) {
	...
	public static Set<Set<OWLClass>> getSuperExplanations(Taxonomy<OWLClass> t, OWLClass sub,
			OWLClass sup) {
		...
		Set<Set<OWLClass>> explanations = map.get( sup );
		if( explanations == null ) {
	        return null;
        }
	 ...
	 *         {@code false} else
	 */
	public static boolean isType(Taxonomy<OWLClass> t, OWLClass ind, OWLClass c) {
		Iterator<Object> i = t.depthFirstDatumOnly( c, INSTANCES_KEY );
		if( !i.hasNext() ) {
	        throw new RuntimeException( c + " is an unknown class!" );
        }
		...
		do {
			Set<OWLClass> instances = (Set<OWLClass>) i.next();
			if( instances != null && instances.contains( ind ) ) {
	            return true;
            }
